...
test('should display circle when data is passed to the panel', async ({
  ...
  await panelEditPage.setVisualization('sogekng-grafanaaccessibilitycolorblind-panel');
...
test('should display series counter when "Show series counter" option is enabled', async ({
  ...
  const options = panelEditPage.getCustomOptions('sogekng-grafanaaccessibilitycolorblind-panel');
